...
const userTypes = [
  {
    ...
    icon: GraduationCap,
    title: "Student",
    description: "Browse and order all products",
    href: "/kiosk/order/browse?type=student",
    accent: "#07484A",
  },
  {
    key: "visitor",
    icon: UserRound,
    title: "Visitor",
    description: "Browse and order limited products",
    href: "/kiosk/order/browse?type=visitor",
    accent: "#6b7280",
  },
];
...
const UserTypePage = () => {
  ...
  const handleSelectType = (type: UserType, href: string) => {
          ...
          <div className="flex w-full flex-col gap-5">
            {userTypes.map(
              ({ key, icon: Icon, title, description, href, accent }) => (
                <button
                  key={key}
                  onClick={() => handleSelectType(key as UserType, href)}
